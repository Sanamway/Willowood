...
const DemoTable = () => {
  ...
  const handleVerify = async () => {
    ...
    try {
      const respond = await axios.put(
        `${url}/api/update_mr_demo_followup/${modalData.id}`,
        JSON.stringify(data),
        {
          headers: headers,
        }
  ...
  const handleDelete = async () => {
    ...
    try {
      const respond = await axios.get(
        `${url}/api/delete_farmer_demo_followup`,
  ...
  const handleApproveAll = async () => {
    ...
    const approvedIds = data.filter(item => item.approved !== "Yes").filter(item => item.isApproved).map(item => item.f_demo_followup_id);
    const newData = {
      approved: "Yes",
      approved_date: new Date(),
    ...
    try {
      ...
      const respond = await axios.post(`${url}/api/update_mr_demo_followup`,
        newData,
        {
          headers: headers,
          params: {
            f_demo_followup_id: JSON.stringify(approvedIds)
          },
  ...
  const handleVerifyAll = async () => {
    const approvedIds = data.filter(item => item.verified !== "Yes").filter(item => item.isVerified).map(item => item.f_demo_followup_id);
  ...
  const handleCheckboxChange = (field, value, item) => {
    setData(prevData =>
      prevData.map(el =>
        el.f_demo_followup_id === item.f_demo_followup_id
          ? { ...el, [field]: value }
          : el
      )
    );
  ...
  const handleApproveAllChange = (e) => {
    ...
    setData(prevData =>
      prevData.map(el => ({
        ...
        isApproved: el.verified === "Yes" ? true : value
      }))
    );
  };
  ...
  return (
    <Layout>
      <div className="absolute h-full overflow-y-auto  mx-4 w-full overflow-x-hidden">
        ...
        <div className="overflow-y-hidden bg-white h-max flex flex-col gap-2  select-none items-start justify-between w-[98%] mx-4 no-scrollbar">
              ...
              {data.map((item, idx) => (
                <tr className="dark:border-2" key={idx}>
                  ...
                  <td
                    className="px-4 py-2 dark:border-2 whitespace-nowrap"
                    onClick={() => {
                      if (item.hand_testimonials_path) {
                        setIMGURL(item.hand_testimonials_path);
                        setShowImageModal(true)
                      }
                    }}
                  >
                    {item.hand_testimonials_path ? (
