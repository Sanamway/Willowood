...
const FieldDay = () => {
  ...
  const handleApproveAll = async () => {
    ...
    const approvedIds = data.filter(item => item.approved !== "Yes").filter(item => item.isApproved).map(item => item.f_demo_fields_id);
    ...
    try {
      ...
      const respond = await axios.post(`${url}/api/update_mr_demo_fieldday`,
        newData,
        {
          headers: headers,
          params: {
            f_demo_fields_id: JSON.stringify(approvedIds)
          },
  ...
  const handleVerifyAll = async () => {
    ...
    const approvedIds = data.filter(item => item.verified !== "Yes").filter(item => item.isVerified).map(item => item.f_demo_fields_id);
  ...
  const handleCheckboxChange = (field, value, item) => {
    setData(prevData =>
      prevData.map(el =>
        el.f_demo_fields_id === item.f_demo_fields_id
          ? { ...el, [field]: value }
          : el
      )
    );
              ...
              {data.map((item, idx) => (
                <tr className="dark:border-2" key={idx}>
                  ...
                  <td
                    ...
                    onClick={() => {
                      if (item.field_day_image_Url) {
                        setImgUrl(item.field_day_image_Url);
                        setIsOpen(true);
                      }
                    }}
                  >
                    {item.field_day_image_Url ? (
