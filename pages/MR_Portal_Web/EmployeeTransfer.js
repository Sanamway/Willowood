...
const EmpoyeeTransffer = () => {
    ...
    const handleUpdate = async () => {
        ...
        const mrList = selectedEmpCodes.map((code) => {
            ...
            return {
                ...
                existingHq: mrData.existingHq || "",
                currentHq: mrData.headquater || "",
        ...
        const dataToSend = selectedEmpCodes.map((empcode) => {
            ...
            return {
                ...
                territory_name: dropdownData.territory_name || "",
    ...
    const handleDropdownChange = (empcode, field, value, territory, existingHq, mrName, number) => {
            ...
            const responses = await Promise.all(
                mrList.map((mr) => {
                    const payload = {
                        ...
                        placeholders: [
                            mr.mrName,
                            `${mr.existingTerritory}/${mr.existingHq}`,
                            `${mr.currentTerritory}/${mr.currentHq}`,
                            mr.date,
                            mr.mrName
                        ]
                    };
                                        ...
                                        <>
                                            <td className="px-4 py-2 dark:border-2 whitespace-nowrap">
                                                <select
                                                    ...
                                                    onChange={(e) =>
                                                        handleDropdownChange(item.empcode, "headquater", e.target.value, item.territory_name, item.reporting_hq, item.fname + item.mname + item.lname, item.phone_number)
                                                    }
