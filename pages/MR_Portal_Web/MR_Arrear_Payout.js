...
const FeePayout = () => {
        ...
        try {
            ...
            const ws = XLSX.utils.json_to_sheet(apires.map((item, idx) => {
                return {
                    ...
                    ["Recon Days"]: item.total_working_day,
                    ["Arrear Days"]: item.arrear_day,
                    ["Absent"]: item.a_count,
        ...
        try {
            const paramsData = data?.map((item, idx) => {
                return {
                    ...
                    arrearsDays: item["Arrear Days"],
                    m_year: item["m_year"]
                }
            }
            );
            const respond = await axios
                .post(`${url}/api/employee_arrearDays`, JSON.stringify(paramsData), {
                    headers: headers,
                })
                .then((res) => {
    ...
    const getNewExcelsheet = async (
        ...
        try {
            ...
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "Sheet1");
            XLSX.writeFile(wb, `download_template_arreardays.xlsx`);
            setExcelLoading(false)
        } catch (error) {
            setExcelLoading(false)
            console.log("notch", error)
        ...
        if (!data.length) {
            ...
            try {
                ...
                if (type === "Yet to Submit") {
                    endPoints = "add_employee_arreardays"
                    ...
                    endPoints = "update_employee_arrear_status"
                    paramsData = data?.map((item) => {
                        return {
            ...
            <div className="absolute h-full overflow-y-auto  mx-4 w-full overflow-x-hidden">
                ...
                <div className="flex flex-row gap-4  px-4 pr-8 pb-2 text-xs">
                    ...
                    >
                        ...
                        {!(userRole === 17 || userRole === 8 || userRole === 18) && (
                            <option value={"3"}>Arrear Days Calculation</option>
                        )}
                        ...
                        <table className="min-w-full divide-y border divide-gray-200">
                            <thead className="border-b">
                                <tr className="bg-gray-50 font-arial">
                                    ...
                                    <th className="px-4 py-2 text-right text-xs font-medium text-gray-500 tracking-wider">Arrear Days</th>
                                    ...
                                    <th className="px-4 py-2 text-right text-x s font-medium text-gray-500 tracking-wider">Earning Salary</th>
                                ...
                                {data?.map((item, idx) => (
                                    <tr className="dark:border-2" key={idx}>
                                        <td className="px-4 py-2 text-left flex flex-row whitespace-nowrap">
                                            {gridDataType === "Yet to Submit" ? <input
                                                type="checkbox"
                                                disabled={item.arrear_status !== "Yet to Submit"}
                                            ...
                                            <span className="ml-2">{gridDataType === "Yet to Submit" ? item.arrear_status : gridDataType}</span>
                                        </td>
                                        ...
                                        <td className="px-4 py-2 text-right">{item.arrear_day}</td>
