...
function BusinessInfoModal({ onClose, isOpen, onOpen, userId, onDeletedData, method, endpoints,secondQUery, d_id, b_id  }) {
  ...
  const methodDelete = async () => {
    try {
      const resp = await axios[method](`${url}/api/${endpoints}?d_id=${d_id}&${secondQUery}=${b_id}`, { headers });
