...
function DocsDeleteModal({ onClose, isOpen, onOpen,successDel, userId, onDeletedData, method, endpoints, deleteType }) {
  ...
  const methodDelete = async (userId) => {
    try {
      const resp = await axios[method](`${url}/api/${endpoints}?file_path=${deleteType}&_id=${userId}`, { headers });
...
export default DocsDeleteModal;
