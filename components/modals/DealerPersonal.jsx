...
function DealerPersonal({ onClose, isOpen, onOpen, userId, onDeletedData, method, endpoints }) {
  ...
  const methodDelete = async (userId) => {
    ...
    try {
      const resp = await axios[method](`${url}/api/${endpoints}?d_id=${userId?.d_id}&p_id=${userId?.p_id}`, { headers });
