...
const TotalOutStandPop = ({ closeModal, regionData, catData, dueData }) => {
  ...
  return (
    <>
      <animated.div className=" bg-gray-100/40  backdrop-blur-[4px] from-gray-10 to-transparent z-50 w-full flex items-center justify-center min-h-screen fixed top-0 right-0 left-0 bottom-0 ">
        ...
        <animated.div style={springProps} className="mainContainer lg:w-[55%] w-full mx-2 h-screen  bg-white rounded-lg top-2 absolute  ">
          ...
          {selected == "category" && (
            <div className="orderwrapper px-2 py-2 overflow-y-auto h-screen pb-24 mb-24 ">
              <div className="w-full px- mt-2 flex lg:flex-row flex-col gap-3 font-arial rounded-md">
                <div className="bg-white  flex-1 md:flex items-center justify-center gap-4 rounded-md shadow- text-white text-center o">
                    ...
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400 rounded-full  ">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200 break-normal ">
                          ...
                          catdata?.filter((item) => {
                            ...
                            return (
                              <tr key={idx}>
                                ...
                                {specialColumn && <td className="px-2 text-center whitespace-nowrap py-1 text-[0.75rem] text-gray-600 border">
                                  {item["121-180"]}
                                </td>}
                                ...
                                <td className="px-2 text-center whitespace-nowrap py-1 text-[0.75rem] text-gray-600 border">
                                  {/* {((specialColumn ? Number(item["121-180"]) || 0 : 0) + Number(item["180-365"]) || 0 + Number(item["366-720"]) || 0 + Number(item["720 And Above"]) || 0)} */}
                                  {(
                                    (specialColumn ? Number(item["121-180"]) || 0 : 0) +
                                    (Number(item["180-365"]) || 0) +
                                    (Number(item["366-720"]) || 0) +
                                    (Number(item["720 And Above"]) || 0)
                                  ).toFixed(2)}
                                </td>
                                <td className="px-2 text-center whitespace-nowrap py-1 text-[0.75rem] text-gray-600 border">
                                  {item.Territory}
                        ...
                        <tr>
                          ...
                          <td className="px-2 text-center whitespace-nowrap py-1 text-[0.75rem] text-gray-600 border font-bold">
                            Total
                          </td>
                          {specialColumn && catdata.reduce((acc, curr) => acc + curr["121-180"], 0) !== 0 && (
                            <td className="px-2 text-center whitespace-nowrap py-1 text-[0.75rem] text-gray-600 border font-bold">
                              {catdata.reduce((acc, curr) => acc + curr["121-180"], 0).toFixed(2)}
                            </td>
                          )}
                          ...
                          <td className="px-2 text-center whitespace-nowrap py-1 text-[0.75rem] text-gray-600 border font-bold">
                            {((specialColumn ? Number(catdata.reduce((acc, curr) => acc + curr["121-180"], 0).toFixed(2)) || 0 : 0) + Number(catdata.reduce((acc, curr) => acc + curr["180-365"], 0).toFixed(2)) + Number(catdata.reduce((acc, curr) => acc + curr["366-720"], 0).toFixed(2)) + Number(catdata.reduce((acc, curr) => acc + curr["720 And Above"], 0).toFixed(2))).toFixed(2)}
                          </td>
                          <td className="px-2 text-center whitespace-nowrap py-1 text-[0.75rem] text-gray-600 border font-bold">
                          </td>
                        </tr>
